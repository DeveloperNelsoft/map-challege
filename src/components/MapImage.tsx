...
const MapImage = compose<any, any>(
  ...
  <GoogleMap zoom={props.maps.zoom} center={{ lat: props.maps.lat, lng: props.maps.lng }} heading={5}>
    ...
    <Marker position={{ lat: props.maps.lat, lng: props.maps.lng }}>
      <InfoWindow>
        <p>
          {!props.maps.cityName.length
            ? 'city?'
            : `city: ${props.maps.cityName} - wind Speed: ${props.maps.windSpeed}  - wind Deg: ${props.maps.windDeg} `}
        </p>
      </InfoWindow>
    </Marker>
  </GoogleMap>
